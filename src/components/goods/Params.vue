...
export default {
  ...
  methods: {
    ...
    async showEditDialog(attr_id) {
      ...
      const { data: res } = await this.$http.get(
        ...
        {
          attr_sel: this.activeName,
          attr_vals: this.manyTableData.attr_vals
        }
      )
    ...
    showInput(row) {
      ...
      // 让文本框自动获得焦点
      // 将row.inputVisible改为true后，模板还未渲染
      // 等渲染后，在回调就可以得到input
      this.$nextTick(_ => {
  ...
  computed: {
    ...
    attrId() {
      // 判断是哪个tab标签的数据
      if (this.activeName === 'many') {
        return this.manyTableData.attr_id
      } else {
        return this.onlyTableData.attr_id
      }
    },
    // 动态计算标题的文本
    titleText() {
